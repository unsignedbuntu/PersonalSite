        ...
        <nav className="flex items-center justify-end gap-4 md:gap-6">
          {navItems.map((item, index) => (
            <div key={item.name} className="flex items-center">
              <motion.a
                href={item.href}
                className={`group font-semibold tracking-wider transition-all duration-300 relative ${
              ...
              >
                ...
                {activeSection === item.id && (
                  <motion.div
                    ...
                    className="absolute -bottom-2 left-0 right-0 h-0.5 bg-gradient-to-r from-yellow-300 to-green-400 rounded-full"
                    initial={{ scaleX: 0 }}
                    animate={{ scaleX: 1 }}
                    transition={{ 
                      type: "spring", 
